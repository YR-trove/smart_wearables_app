...
class _ConnectionPageState extends State<ConnectionPage> {
  ...
  void connectionProcedure(String id, ConnectionStateUpdate event){ 
    ...
    // --- 3. Navigation ---
    // After a successful BLE connection, push MainShell so the user lands on
    // the full app with bottom navigation (Fitness, Light, Stress, Settings).
    // The Developer tab (raw sensor dashboard) inside MainShell is gated behind
    // Developer Mode and receives the live incomingBLEStream.
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
